...
export const EditAddressForm = ({ defaultValues }: EditAddressFormProps) => {
	...
	const { control, handleSubmit, formState, reset, setFocus, setValue, watch } = form
								...
								<Controller
									...
									render={({ field, fieldState }) => (
										<Field data-invalid={fieldState.invalid} className="md:col-span-1">
											...
											<Input {...field} id={cepId} placeholder="00000-000" aria-invalid={fieldState.invalid} onChange={(event) => handleCepChange(event.target.value, field.onChange)} onBlur={handleCepBlur} />
								...
								<Controller
									...
									render={({ field, fieldState }) => (
										<Field data-invalid={fieldState.invalid}>
											...
											<Select onValueChange={(val) => handleStateChange(val, field.onChange)} value={field.value} disabled={isFetchingCep}>
