...
type GetOrganizationUsersContextActionRes = {
	organization: {
		name: string
	}
...
export async function getOrganizationUsersContextAction(): OperationResponse<GetOrganizationUsersContextActionRes, ErrorCodes> {
	const useCaseRes = await getOrganizationUsersContextUseCase()
	...
	const users: OrganizationUserTableRow[] = useCaseRes.data.organizationMembers.map((member) => {
	...
	return {
		...
		message: useCaseRes.message,
		data: {
			organization: useCaseRes.data.organization,
			permissionKeys: useCaseRes.data.permissionKeys,
			roles,
			users
		}
	}
