// @/modules/fleet/shared/ui/register-as-driver-form.tsx

"use client"

import { zodResolver } from "@hookform/resolvers/zod"
import { ArrowLeft, ArrowRight, AtSign, Car, FileText, Flag, Hash, House, IdCard, Lock, Mail, MapPin, Palette, Phone, User, UserPlus } from "lucide-react"
...
import { VEHICLE_TYPE_LABELS, VEHICLE_TYPES } from "@/modules/fleet/shared/constants/vehicle-types"
import { type RegisterAsDriverSchemaClientData, registerAsDriverSchemaClient } from "@/modules/fleet/shared/validations/register-as-driver.schema"
import { DRIVER_DOCUMENT_MIME_TYPES } from "@/modules/fleet/shared/validations/vehicle.schema"
...
export const RegisterAsDriverForm = () => {
	const baseId = useId()
	const formId = `${baseId}-register-as-driver-form`
	...
	// Propaga o ?ref= da URL para a action (indicação de quem convidou o motorista).
	const ref = searchParams.get("ref")?.trim() || undefined
	const showRelationshipField = Boolean(ref)

	const registerAsDriverForm = useForm<RegisterAsDriverSchemaClientData>({
		// zodResolver infere o tipo de INPUT do schema; como os campos de veículo usam
		// coerce/preprocess, o input diverge do output. Fixamos no tipo de output.
		resolver: zodResolver(registerAsDriverSchemaClient) as unknown as Resolver<RegisterAsDriverSchemaClientData>,
		defaultValues: {
			name: "",
			username: "",
			phone: "",
	...
	const { control, handleSubmit, formState, setValue, setFocus, trigger, resetField, reset, watch } = registerAsDriverForm
	...
	async function nextStep(currentStep: number) {
		let fieldsToValidate: FieldPath<RegisterAsDriverSchemaClientData>[] = []
	...
	async function onSubmit(data: RegisterAsDriverSchemaClientData) {
		try {
			...
			console.error("[registerAndJoinAsDriverAction] erro inesperado:", error)
			...
			<CardHeader>
				<div className="flex w-full items-start pt-6">
					...
					<div className="flex flex-1 flex-col items-center">
						...
						<p className={cn("mt-2 text-sm font-medium", step >= 3 ? "text-primary" : "text-muted-foreground")}>Veiculo</p>
					</div>
				</div>
			</CardHeader>
			...
			<CardContent>
				<form id={formId} onSubmit={handleSubmit(onSubmit)} className="space-y-6">
							...
							<FieldGroup>
								<div className="grid grid-cols-1 gap-6 md:grid-cols-2">
									...
									<Controller
										...
										render={({ field, fieldState }) => (
											<Field data-invalid={fieldState.invalid}>
												<FieldLabel htmlFor={vehicleTypeId}>Tipo de veiculo</FieldLabel>
												<Select onValueChange={field.onChange} value={field.value ?? ""}>
										...
										render={({ field, fieldState }) => (
											<Field data-invalid={fieldState.invalid}>
												...
												<InputGroup>
													...
													<InputGroupInput {...field} id={vehicleModelId} placeholder="Fiat Strada" aria-invalid={fieldState.invalid} />
												</InputGroup>
												{fieldState.invalid && <FieldError errors={[fieldState.error]} />}
											</Field>
										)}
										...
										render={({ field, fieldState }) => (
											<Field data-invalid={fieldState.invalid}>
												...
												<InputGroup>
													...
													<InputGroupInput {...field} id={vehicleColorId} placeholder="Branco" aria-invalid={fieldState.invalid} />
												</InputGroup>
												{fieldState.invalid && <FieldError errors={[fieldState.error]} />}
											</Field>
										)}
										...
										render={({ field, fieldState }) => (
											<Field data-invalid={fieldState.invalid}>
												<FieldLabel htmlFor={crlvId}>
													<FileText className="mr-2 h-4 w-4" /> CRLV do veiculo
												</FieldLabel>
												<Input id={crlvId} type="file" accept={DOCUMENT_ACCEPT} aria-invalid={fieldState.invalid} onChange={(e) => field.onChange(e.target.files?.[0])} />
												<FieldDescription>PDF, JPG, PNG ou WEBP (max 10 MB).</FieldDescription>
												{fieldState.invalid && <FieldError errors={[fieldState.error]} />}
											</Field>
										)}
									/>
									<Controller
										...
										render={({ field, fieldState }) => (
											<Field data-invalid={fieldState.invalid}>
												...
												<Input id={cnhId} type="file" accept={DOCUMENT_ACCEPT} aria-invalid={fieldState.invalid} onChange={(e) => field.onChange(e.target.files?.[0])} />
												<FieldDescription>PDF, JPG, PNG ou WEBP (max 10 MB).</FieldDescription>
												{fieldState.invalid && <FieldError errors={[fieldState.error]} />}
											</Field>
										)}
					...
					<div className="flex justify-between pt-4">
						{step > 1 && (
							<Button type="button" variant="outline" onClick={prevStep} disabled={isFetchingUserCep || formState.isSubmitting}>
