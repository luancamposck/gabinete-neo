import { listSurveyQuestionsWithOptionsRepo } from "@/modules/surveys/server/repos/list-survey-questions-with-options.repo"
...
export async function listSurveyQuestionsWithOptionsService(params: { surveyId: string }): OperationResponse<{ questions: SurveyQuestionWithOptions[] }, "infra_error"> {
	const { data, error } = await listSurveyQuestionsWithOptionsRepo(params)

	if (error) {
		console.error("[listSurveyQuestionsWithOptionsService]:", error.message)
		return {
			success: false,
