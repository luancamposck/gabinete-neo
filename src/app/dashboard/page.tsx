      ...
      <div className="grid grid-cols-1 @xs:grid-cols-2 @xl:grid-cols-3 @6xl:grid-cols-5  gap-4">
        {STATS.map(({ icon: Icon, ...cardContent }) => (
          <Card key={cardContent.id} className="@max-md:gap-2 @max-md:py-2">
            ...
            <CardFooter className="text-emerald-500 dark:text-emerald-400">
              {cardContent.change}
            </CardFooter>
          </Card>
        ))}
            ...
            <ul className="space-y-4">
              {RECENT_ACTIVITIES.map((ACTIVITY) => (
                <li key={ACTIVITY.id} className="flex items-start gap-3">
                  <div
                    className={cn("size-2 rounded-full mt-2", ACTIVITY.color)}
                  />
                ...
                return (
                  ...
                  >
                    <p className="text-[#F8FAFC] text-sm">
                      Votação importante sobre transporte público amanhã
                    </p>
                    <Badge className={cn("mt-2 text-xs", color.badge)}>
                      MEDIA
                    </Badge>
                  </li>
                )
