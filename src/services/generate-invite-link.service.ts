...
export async function generateInviteLinkService(
  params: GenerateInviteLinkServiceParams
): Promise<OperationResponse<{ inviteUrl: string }>> {
  ...
  } = await findOrganizationMembershipByUserAdminRepo({userId: inviterUserId})
  ...
  if (membership.organization_id !== organizationId) {
    console.error(
      "[generateInviteLinkService] organizationId da página não bate com membership:",
      { fromPage: organizationId, fromMembership: membership.organization_id }
    )
