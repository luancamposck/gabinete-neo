...
export async function getUserOrganizationService(): Promise<
  OperationResponse<GetUserOrganizationServiceData>
> {
  ...
  } = await findOrganizationMembershipByUserAdminRepo({userId})

  if (membershipError || !membership) {
    console.error(
      "[getUserOrganizationService] Membership não encontrado para user:",
      userId,
      membershipError
    )
  ...
  } = await findOrganizationByIdRepo(organizationId)

  if (organizationError || !organization) {
    console.error(
      "[getUserOrganizationService] Organização não encontrada para id:",
      organizationId,
      organizationError
    )
